...
namespace HV
{
    ...
    inline ULONG64 CallVM(ULONG64 arg1 = 0, ULONG64 arg2 = 0, ULONG64 arg3 = 0, ULONG64 arg4 = 0)
    {
        if (!Data::Shellcode)
        {
            // vmcall
            // ret
            const BYTE code[] = { 0x0F, 0x01, 0xC1, 0xC3 };

            // Allocate as RW, write code, then change to RX (avoid RWX pages — BE scans for them)
            Data::Shellcode = VirtualAlloc(nullptr, 0x1000, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
            if (!Data::Shellcode)
            ...
            DWORD oldProtect;
            VirtualProtect(Data::Shellcode, 0x1000, PAGE_EXECUTE_READ, &oldProtect);
        }
    ...
    inline bool AttachToProcess(UINT32 processId)
    {
        ...
        // NOTE: No OpenProcess on target — we don't need a handle.
        // All memory access goes through the hypervisor via CR3.
        // Opening a handle to the game process is monitored by BE.
